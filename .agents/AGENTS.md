...
- **github-sub-issues:** Use when creating sub-issues or sub-tasks for a feature, or breaking work into per-repository work items — the tracking-issue pattern: a parent tracking issue with a mandatory Sub-issues table (`| Repository | Issue | Scope |`) plus one self-contained sub-issue per repository, each opening with a Context sentence naming the parent feature and repository.
