...
- **coding-computesharp:** Use when writing or changing a ComputeSharp `IComputeShader` — constant buffer layout and field order, the NVIDIA odd root constant + odd resource count defect (reorder or pad to an even constant count), the `ComputeShader_ConstantBufferLayout` guard and parity facts, and probing a suspected GPU defect.
...
- **github-plan-files:** Use when an implementation plan exists or a commit/push is about to happen in a DiGi repository — plans live in the local plans folder outside the working tree, a pre-commit sweep keeps plan/scratch `*.md` out of every commit and push, and the default is deleting the temporary implementation plan when the issue closes (no archiving into the repo; explicit user instruction wins).
- **github-sub-issues:** Use when creating sub-issues or sub-tasks for a feature, or breaking work into per-repository work items — the tracking-issue pattern: a parent tracking issue with a mandatory Sub-issues table (`| Repository | Issue | Scope |`) plus one self-contained sub-issue per repository, each opening with a Context sentence naming the parent feature and repository; sibling ordering recorded as GitHub issue dependencies.
